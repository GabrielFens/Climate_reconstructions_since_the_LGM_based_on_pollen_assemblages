...
pollen=read.csv2("AssFLGP.csv",sep=';',dec=',') 
pollen=pollen[,-c(1)]
pollen=pollen*100
...
#Homogénéisation LGP1
for (i in 1:nrow(pollen))
{pollen[i,"Quercus ilex-type"]=pollen[i,"Quercus ilex-type"]+pollen[i,"Quercus robur-type"]
pollen[i,"Asteraceae subf. Asteroideae"]=pollen[i,"Asteraceae subf. Asteroideae"]+pollen[i,"Asteraceae subf. Cichorioideae"]}
colnames(pollen)[colnames(pollen)=="Quercus ilex-type"]="Quercus"
colnames(pollen)[colnames(pollen)=="Acer campestre-type"]="Acer"
colnames(pollen)[colnames(pollen)=="Asteraceae subf. Asteroideae"]="Asteraceae"
colnames(pollen)[colnames(pollen)=="Caryophyllaceae (periporate excl. Paronychioideae)"]="Caryophyllaceae"
colnames(pollen)[colnames(pollen)=="Chenopodiaceae/Amaranthaceae"]="Amaranthaceae"
colnames(pollen)[colnames(pollen)=="Ranunculus-type"]="Ranunculaceae"

#Homogénéisation LGP2
colnames(pollen)[colnames(pollen)=="Chenopodiaceae/Amaranthaceae"]="Amaranthaceae"

#Homogénéisation Furamoos
colnames(pollen)[colnames(pollen)=="Acer campestre-type"]="Acer"
colnames(pollen)[colnames(pollen)=="Caryophyllaceae (periporate excl. Paronychioideae)"]="Caryophyllaceae"
colnames(pollen)[colnames(pollen)=="Ranunculus acris type"]="Ranunculaceae" 
...
colnames(pollen)[colnames(pollen)=="Chenopodiaceae/Amaranthaceae"]="Amaranthaceae"
colnames(pollen)[colnames(pollen)=="Acer campestre-type"]="Acer"
colnames(pollen)[colnames(pollen)=="Quercus robur-type"]="Quercus"
...
for(i in 1:sample.no){res[i,"Best"]<- biomenames[which.max(res[i,biomenames])]
res[i,"Best.num"]<- which.max(res[i,biomenames])}
write.csv(res, file="OutModern_MegaBiomization_Modern2.csv")
write.csv(Loc, file="PositionSiteCal_Output2.csv")
