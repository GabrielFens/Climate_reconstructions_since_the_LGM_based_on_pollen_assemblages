...
EMPD2=read.csv2("Plantago_lanceolata-type_Add.txt",sep='\t',dec=",")
#Plantago lanceolata
...
PA=PA[,-1]
PA1=PA
BPFT=read.csv2("biompft-23.txt",sep='\t',dec=",")
Name_Biome=BPFT[,1]
...
AssPol=read.csv2("AssFEifel.csv",sep=';',dec=',') 
AssPol=AssPol[,-c(1)]
AssPol=AssPol*100
colnames(AssPol)=Name$Names
AgesCal=read.csv2("ModeleAge_Eifel.csv",sep=';',dec='.')
Ages=AgesCal$Age
#AgesCal=data.frame(matrix(NA,ncol=1,nrow=nrow(AssPol)))
#Ages=AgesCal$matrix.NA..ncol...1..nrow...nrow.AssPol..
namefile="OutEifel_Biomisation_2"

EuropeBiomeIndex<-function(Name,AssPol,Ages,namefile,j) {
if (j==1)
{#Initialization of output results: the biome returned, dominated affinity score and score dispersion over PFT
Output=data.frame(matrix(NA,ncol=3,nrow=nrow(SiteNameCal)))
dimnames(Output)=list(c(SiteNameCal$Name),c("Biome","Score","Std"))
...
OutBiomizationFuramoos_2=read.csv2("OutFuramoos_Biomisation_2.csv")
...
Age=Age_LDB$Age_Bacon
OutBiomization=OutBiomizationLDB_1[,-c(1)]
...
data=data.frame(time/1E3,Biome,table)
library(ggplot2)
library(dplyr)
ggplot(data,aes(time/1E3,table,fill=Biome))+
  geom_area()+
  scale_fill_manual(values=c('#EE82EE','blue','darkolivegreen',"#CAFFCA",'cyan','green','#BBCB35','grey',"#AE7D20", '#0020CA',"#55EB49","#000065",'#65B2FF',"#E7E718","#E69F00"))+
...
OutBiomization1=read.csv2("OutModern_Biomization_2.csv",sep=';',dec='.')
OutBiomization2=read.csv2("OutModern_Biomization_1.csv",sep=';',dec='.')
...
OutBiomization=OutBiomization1
...
barplot(table(BiomeNum),col=c('#65B2FF','#BBCB35',"#E69F00","cyan",'#0020CA',"#55EB49","#CAFFCA","#000065","#E7E718","#91283B","grey",'#EE82EE','blue','darkolivegreen'))
boxplot(BiomeNum)
...
ggplot(DistLat,aes(y=ModernClimate.T_ann,x=BiomeNum.BiomeNum,fill=factor(BiomeNum.BiomeNum))) +
  geom_boxplot(width = 0.3)+
  scale_fill_manual(values=c('#65B2FF','#BBCB35',"#E69F00","cyan",'#0020CA',"#55EB49","#CAFFCA","#000065","#E7E718","#91283B","grey",'#EE82EE','blue','darkolivegreen'))+
  theme_classic()
...
map('worldHires', col='gray90', fill=T,xlim=c(-60,200), ylim=c(10,90))+
  points(Samples$Longitude,Samples$Latitude, col=BiomeNum,cex=2)+
  legend("bottomright", cex = 0.7, bty = "n")
library(tidyverse) 

data=subset(DataSynthesis)
...
geom_map( 
  ...
  scale_color_manual(values=c('#65B2FF',"#332288","#B2912f","cyan","#CC79A7","#CAFFCA","#117733",'yellow',"#E69F00",'#0b53c1','orange',"grey","blue",'#EE82EE'))+
